...
def generate_true_data():
    ...
    #add gausian noise
    data_noisy = data + np.random.normal(0, 3, data.shape)
    return data_noisy

#function generates random gaussian noise
def generate_false_data():
    data = np.zeros(500000, dtype=float)
    data_noisy = data + np.random.normal(0, 3, data.shape)
    return data_noisy
...
#save training data
np.save("training-3-data.npy", training_data_array)
np.save("training-3-labels.npy", training_label_array)
...
#save testing data
np.save("testing-3-data.npy", testing_data_array)
np.save("testing-3-labels.npy", testing_label_array)
